...
│   ├── dashboard/
│   │   ├── templates/          # Jinja2 HTML — index.html is the SPA; cbee/gadcf/fhim/ashrta.html are legacy redirects
│   │   └── static/             # JS/CSS (Chart.js, Leaflet.js, WebSocket client)
│   ├── config/                 # Per-service config files (cowrie.cfg, userdb.txt, dionaea.cfg,
...
└── neurotrap-db/               # MongoDB schema artifact repo (Mongoose / mongosh only — no Python)
    ...
    └── scripts/init_db.js      # mongosh init: creates every collection with validator + indexes
```
...
### Full stack (Docker — Ubuntu 22.04)

```bash
cd neurotrap
cp .env.example .env                    # fill MONGO_PASS, SECRET_KEY, JWT_SECRET, MONITOR_INTERFACE
...
## Architecture — Data Flow (11 layers)
...
Internet ──► Honeypots (Layer 1)
         ...
         ASHRTA Hardening (Layer 9)    →  ashrta_reports, ashrta_patches
              ↓  threat_score / response decision
         Response Engine (Layer 10)    →  response_log
              ↓
         Flask API + WebSocket (Layer 11) → Dashboard SPA
```
...
### `src/detection/`
- **`alert_schema.py`** — `AlertEvent` dataclass: `src_ip, dst_port, attack_type, honeypot_source, severity, command, raw_payload`. Factory methods `from_cowrie()` and `from_zeek()` normalize external log formats.
...
- **`log_pipeline.py`** — `LogIngestionPipeline`: background threads tail Cowrie/Dionaea JSON log files and emit `AlertEvent` objects to `alert_events`.
- **`main.py`** — starts both `PacketMonitor` and `LogIngestionPipeline` as threads.

### `src/behavior/`
- **`classifier.py`** — `AttackerClassifier`: `VotingClassifier(RF + SVM)`. Input: 13-dim feature vector. Outputs: intent label (6 classes: `reconnaissance`, `credential_harvesting`, `malware_deployment`, `lateral_movement`, `cryptomining`, `bot_enrollment`) + tier (`beginner`, `automated_bot`, `advanced_human`) + confidence. F1 macro > 0.85 target.
- **`behavior_engine.py`** — `SessionFeatureExtractor`: converts Cowrie session dict → 13-dim vector (command counts, dangerous/recon indicators, duration, login attempts, persistence/lateral signs). `BehaviorEngine`: background loop polling unprocessed sessions.
...
- **`main.py`** — entry point: polls `attacker_profiles` for unprocessed profiles and spawns environments.
...
- **`cbee_engine.py`** — `CBEEEngine`: re-scores active sessions; fires bait injection when overall ≥ 25 and < 3 injections already sent.
...
- **`kill_chain.py`** — `build_kill_chain()`: maps MITRE tactics → 7-stage Lockheed Martin Kill Chain (Recon → Weaponization → Delivery → Exploitation → Installation → C&C → Actions on Objectives).
...
- **`aggregation_server.py`** — `FedAvgServer`: collects deltas from ≥ 2 nodes; averages weights via numpy; returns updated global model. Pre-seeded with 4 demo orgs (Cairo Uni, Acme Financial, Fraunhofer FKIE, SaudiTelecom).

### `src/ashrta/` — Autonomous Self-Hardening Red-Team Adversarial
- **`red_agent.py`** — `RedAgent`: 10 honeypot-fidelity checks (timing analysis, SSH banner fingerprint, OS/kernel consistency, filesystem completeness, process tree plausibility, network stack fingerprint, service response behavior, CPU/memory artifacts, uptime, error message authenticity). Returns `DetectionResult` per check.
- **`hardening_optimizer.py`** — `HardeningOptimizer`: generates patches from failed checks targeting `cowrie.cfg`, `env_template`, `honeyfs`, `network`. `ASHRTAScheduler`: orchestrates full red-team + hardening cycle.

### `src/soc_analyst/`
- **`soc_analyst.py`** — `SOCAnalyst`: triage queue (ranks active IPs by threat_score with risk band + recommended action), per-attacker incident report, analyst Q&A. Works offline with heuristic text or with `ANTHROPIC_API_KEY` for LLM output.
...
### `src/api/app.py`
Flask + Flask-SocketIO. All innovation engines are lazy-loaded by `_get_<name>()` factories that return `None` on import failure and serve demo data instead.

**Key REST endpoints:**

| Route | Method | Notes |
|-------|--------|-------|
| `/api/auth/login` | POST | Returns JWT (`flask-jwt-extended`; falls back to dummy token) |
| `/api/events` | GET | Query `alert_events`; filters: `attack_type`, `severity`; sorted by `timestamp` DESC |
| `/api/events/stats` | GET | Aggregation: total events, active sessions, blocked IPs, by-type breakdown |
| `/api/attackers` | GET | Top profiles by `threat_score` DESC |
| `/api/attackers/<src_ip>` | GET | Single profile |
| `/api/response/block` | POST | Manual iptables block (auth required) |
| `/api/environments` | GET | Active deception environments |
| `/api/honeypots` | GET | Sensor hit counts + unique attacker IPs + recent sessions |
| `/api/cbee/profiles` | GET | Bias profiles (top 50) |
| `/api/cbee/score` | POST | Ad-hoc session scoring |
| `/api/gadcf/generate` | POST | Trigger content generation (industry, intent, sophistication) |
| `/api/twin/list` | GET | All digital twins |
| `/api/twin/simulate` | POST | N-step forward simulation (live twin or ad-hoc tactic sequence) |
| `/api/soc/summary` | GET | Shift summary |
| `/api/soc/triage` | GET | Ranked action queue |
| `/api/soc/report` | POST | Generate LLM incident report for one IP |
| `/api/soc/chat` | POST | Analyst Q&A |
| `/api/ashrta/run` | POST | Trigger red-team + hardening cycle |
...
`alert_events`, `honeypot_sessions`, `cowrie_sessions`, `attacker_profiles`, `attacker_twins`, `cbee_profiles`, `cbee_injections`, `gadcf_assets`, `fhim_rounds`, `fhim_aggregation_rounds`, `deception_environments`, `response_log`, `soc_reports`, `ashrta_reports`, `ashrta_patches`
...
|---------|-------------------|-------|-------|
| `cowrie` | `cowrie/cowrie:latest` | 22→2222, 23→2223 | SSH/Telnet emulator |
| `dionaea` | `dinotools/dionaea:latest` | 21, 80, 445, 3306 | Malware collector |
| `opencanary` | `Dockerfile.opencanary` | 1433, 161/udp, 5900, 3389 | Multi-service (MSSQL, SNMP, VNC, RDP) |
| `galah` | `0x4d31/galah:latest` | 8088→8080 | LLM web honeypot (Anthropic/OpenAI) |
| `honeypots` | `Dockerfile.honeypots` | 2222, 2323, 2121, 8081 | Native Python — opt-in profile `native-honeypots` |
| `mongodb` | `mongo:6.0` | 27017 (internal) | `neurotrap` DB |
| `packet-monitor` | `Dockerfile.monitor` | host network | Scapy; needs `NET_ADMIN`+`NET_RAW` caps |
| `behavior-engine` | `Dockerfile.behavior` | — | Polling loop |
| `deception-engine` | `Dockerfile.deception` | — | Needs `/var/run/docker.sock` |
| `api` | `Dockerfile.api` | 5000 (internal) | Flask + SocketIO |
| `nginx` | `nginx:alpine` | 443, 8080→80 | SSL termination |

Networks: `honeypot-net` (172.20.0.0/24, internet-facing) · `elk-net` (internal, honeypots ↔ backends) · `management-net` (internal, backends ↔ API ↔ Nginx).
...
| `ADMIN_USER` / `ADMIN_PASS` | `admin` / `neurotrap2024` | Dashboard login credentials |
...
| `ANTHROPIC_API_KEY` | unset | Enables LLM features (Galah web honeypot, SOC Analyst) |
...
| `test_classifier.py` | Feature extraction, training, prediction, tier classification |
