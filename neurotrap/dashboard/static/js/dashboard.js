...
// ── ASHRTA ────────────────────────────────────────────────────────────────────
let _ashtaLatestReport = null;

async function loadASHRTA() {
  try {
    const [repRes, patchRes] = await Promise.all([
      fetch('/api/ashrta/reports'),
      fetch('/api/ashrta/patches'),
    ]);
    ...
    if (reports.length) {
      _ashtaLatestReport = reports[0];
      updateASHRTAScore(reports[0]);
      renderASHRTAHistory(reports);
    }
    renderASHRTAPatches(patches);
  } catch (e) {}
}

async function ashtaRunCycle() {
  ...
  try {
    ...
    const data = await res.json();
    updateASHRTAScore(data);
    if (data.full_results) renderASHRTAChecks(data.full_results);
    await loadASHRTA();
  } catch (e) {}

  btn.disabled = false;
  btn.innerHTML = '<i class="fa-solid fa-play"></i> Run Cycle';
}

function updateASHRTAScore(report) {
  const score = report.hardening_score || 0;
  document.getElementById('ashrta-score-val').textContent = score.toFixed(0) + '%';
  drawASHRTAGauge(score);

  const summary = document.getElementById('ashrta-summary');
  if (summary) {
    summary.innerHTML = `
      <div class="ashrta-summary-row"><span class="ashrta-summary-label">Checks Passed</span><span class="ashrta-summary-val" style="color:#10b981">${report.checks_passed||0} / ${report.checks_total||10}</span></div>
      <div class="ashrta-summary-row"><span class="ashrta-summary-label">Critical Weaknesses</span><span class="ashrta-summary-val" style="color:${(report.critical_weaknesses||0)?'#ef4444':'#10b981'}">${report.critical_weaknesses||0}</span></div>
      <div class="ashrta-summary-row"><span class="ashrta-summary-label">Patches Applied</span><span class="ashrta-summary-val" style="color:#00d4ff">${report.patches_applied||0}</span></div>
    `;
  }

  if (report.full_results) renderASHRTAChecks(report.full_results);
}

function renderASHRTAHistory(reports) {
  const el = document.getElementById('ashrta-history-bars');
  if (!el) return;
  el.innerHTML = '';
  const maxScore = Math.max(...reports.map(r => r.hardening_score||0), 1);
  [...reports].reverse().forEach(r => {
    const score = r.hardening_score || 0;
    const pct   = (score / 100) * 40;
    const color = score >= 85 ? '#10b981' : score >= 70 ? '#f59e0b' : '#ef4444';
    const bar = document.createElement('div');
    bar.className = 'ashrta-history-bar';
    bar.style.height  = `${pct}px`;
    bar.style.background = color;
    bar.style.minHeight  = '4px';
    bar.title = `${score.toFixed(0)}% — ${timeSince(r.timestamp)}`;
    bar.dataset.score = score.toFixed(0);
    el.appendChild(bar);
  });
}

function renderASHRTAChecks(results) {
  const el = document.getElementById('ashrta-checks-list');
  if (!el) return;
  el.innerHTML = '';
  results.forEach(r => {
    const item = document.createElement('div');
    item.className = `ashrta-check ${r.passed ? 'passed' : 'failed'}`;
    item.title = r.description || '';
    item.innerHTML = `
      <span class="ashrta-check-icon">
        <i class="fa-solid ${r.passed ? 'fa-check' : 'fa-xmark'}" style="color:${r.passed?'#10b981':'#ef4444'}"></i>
      </span>
      <span class="ashrta-check-name">${(r.check_name||'').replace(/_/g,' ')}</span>
      <span class="ashrta-check-sev ${r.severity}">${r.severity}</span>
      <span style="font-size:9px;color:var(--text-muted)">conf: ${((r.confidence||0)*100).toFixed(0)}%</span>
    `;
    el.appendChild(item);
  });
}

function renderASHRTAPatches(patches) {
  const tbody = document.getElementById('ashrta-patches-tbody');
  if (!tbody) return;
  tbody.innerHTML = '';
  if (!patches.length) {
    tbody.innerHTML = '<tr class="empty-row"><td colspan="6"><i class="fa-solid fa-circle-info"></i> No patches yet — run a cycle</td></tr>';
    return;
  }
  patches.forEach(p => {
    const tr = document.createElement('tr');
    ...
    tr.innerHTML = `
      <td style="color:var(--text)">${(p.weakness||'').replace(/_/g,' ')}</td>
      <td><span style="color:${sevColor};font-weight:700;font-size:10px;text-transform:uppercase">${p.severity}</span></td>
      <td style="color:var(--text-dim);font-family:var(--font-mono);font-size:10px">${p.config_target}</td>
      <td style="font-family:var(--font-mono);font-size:10px;color:var(--text-muted);max-width:140px;overflow:hidden;text-overflow:ellipsis">${p.before}</td>
      <td style="font-family:var(--font-mono);font-size:10px;color:#10b981;max-width:140px;overflow:hidden;text-overflow:ellipsis">${p.after}</td>
      <td><span style="color:#10b981;font-size:10px"><i class="fa-solid fa-check"></i> Applied</span></td>
    `;
    tbody.appendChild(tr);
  });
}

function drawASHRTAGauge(score) {
  const canvas = document.getElementById('ashrta-gauge');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  const dpr = window.devicePixelRatio || 1;
  canvas.width = 160 * dpr; canvas.height = 90 * dpr;
  canvas.style.width = '160px'; canvas.style.height = '90px';
  ctx.scale(dpr, dpr);

  const cx = 80, cy = 82, r = 60;
  ctx.beginPath(); ctx.arc(cx, cy, r, Math.PI, 2*Math.PI);
  ctx.lineWidth = 12; ctx.strokeStyle = 'rgba(255,255,255,0.05)'; ctx.lineCap = 'round'; ctx.stroke();

  if (score > 0) {
    const color = score >= 85 ? '#10b981' : score >= 70 ? '#f59e0b' : '#ef4444';
    const endA  = Math.PI + (score/100) * Math.PI;
